...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (TypeEvents.data !== null) {
        ...
        if (event.EventStageDesc && typeof event.EventStageDesc === 'object' && 'code' in event.EventStageDesc) {
            // Create a new EventType object without modifying the original data
            const maskedEventType = { ...(event.EventStageDesc as { code?: string }), code: '********' };
            // Create a new event object with the masked EventType
            return { ...event, EventStageDesc: maskedEventType };
        }
        return event;
        });
