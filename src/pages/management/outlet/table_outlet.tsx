  ...
  const renderRow = (item: any) => {
    return(
      <>
        ...
        <td className='py-4 px-6 text-sm font-bold'>{item.outletName}</td>
        <td className='py-4 px-6 text-sm'>{item.outletLine.name}</td>
        <td className='py-4 px-6 text-sm'>
          <span className="text-sm text-indigo-700">
            {`${item.outletAddress}, ${item.outletCity}, ${item.outletProvince}, ${item.outletZip}`}
          </span>
        </td>
